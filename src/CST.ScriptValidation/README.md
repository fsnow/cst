...
Test using the curated test words:
...
This tests ~2,400 carefully selected words — `syllable-test-words.txt` (every unique syllable pattern) plus `vowel-test-words.txt` (every vowel-hiatus combination) — across all 14 scripts.
...
### Vowel Test Words (`vowel-test-words.txt`)
A complementary curated file giving complete coverage of **vowel hiatus** — independent
vowels in mid-word position — which the syllable set covered only partially. Each entry is
a real, correct corpus word for one unique vowel-hiatus pattern:
- Consonant (inherent `a`) + independent vowel (e.g. नउ, गइ)
- Dependent vowel (matra) + independent vowel (e.g. विआ, महाओघं)
- Independent vowel + independent vowel (e.g. अआ, एओ)
- Runs of 2+ independent vowels, and the niggahita variants of the above

These patterns expose transliteration-scheme ambiguities: 13 of 14 scripts round-trip them
losslessly, but **Cyrillic** cannot disambiguate several (a scheme limitation, not a bug).

Generated by scanning the corpus for vowel-hiatus patterns and selecting one correct
representative per pattern; words exhibiting doubled vowels or corruption are excluded and
listed separately in `reports/vowel-hiatus-candidate-errors.md` for source-text review.
...
All 14 scripts are validated by round-trip conversion (Deva → IPE → Script → IPE → Script):

- Bengali, Cyrillic, Devanagari, Gujarati, Gurmukhi, Kannada, Khmer, Latin, Malayalam, Myanmar, Sinhala, Telugu, Thai, Tibetan

13 of 14 round-trip the curated set losslessly. **Cyrillic** has a small number of failures
