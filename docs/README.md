...
Corpus reference tables, external resources and CST4 feature references:

- [Page numbering by book](reference/PAGE_NUMBERING_BY_BOOK.md) — which of the five print editions (VRI, Myanmar, PTS, Thai, Other) each of the 217 books is paginated to, and what that means for Go To, the status bar and citable page references. Generated by `reference/page_numbering_by_book.py`; re-run after a corpus update rather than editing
...
- [Pāli fidelity cases](testing/PALI_FIDELITY_CASES.md) - the fixed eval set for surface B output: Pāli questions with authoritative answers, tracked as durable cases with dated per-model observations (grounded vs not)
